...
export class MTFAnalyzer {
  ...
  _evaluateFatRedistribution(measurement) {
    ...
    if (measurement.hips && measurement.thigh) {
      details.lowerBodyDevelopment = '✓ 하체 발달 측정 중';
    }
  ...
  analyzeBodyReshaping() {
    ...
    if (latest.waist && first.waist) {
      changes.waist = {
        ...
        evaluation: latest.waist < first.waist ? '✅ 감소 중 (좋음)' : '⚠️ 증가 중'
      };
    }
    ...
    if (latest.hips && first.hips) {
      changes.hips = {
        ...
        evaluation: latest.hips > first.hips ? '✅ 증가 중 (좋음)' : '⚠️ 감소 중'
      };
    }
    ...
    if (latest.thigh && first.thigh) {
      changes.thigh = {
        ...
        evaluation: latest.thigh > first.thigh ? '✅ 증가 중 (좋음)' : '⚠️ 감소 중'
      };
    }
    ...
    if (latest.waist && latest.hips && first.waist && first.hips) {
      ...
      changes.whr = {
        ...
        evaluation: latestWHR < firstWHR ? '✅ 여성적으로 변화 중' : '⚠️ 개선 필요'
      };
    }
  ...
  _evaluateOverallReshaping(changes) {
    ...
    Object.values(changes).forEach(change => {
      if (change.evaluation && change.evaluation.startsWith('✅')) {
        positiveChanges++;
      }
      totalChanges++;
    });
    ...
    if (percentage >= 80) {
      return '🎉 훌륭합니다! 신체가 여성적으로 재형성되고 있습니다!';
    } else if (percentage >= 60) {
      return '✨ 좋은 진행 상황입니다. 계속 유지하세요!';
    } else if (percentage >= 40) {
      return '📈 진행 중입니다. 운동과 식단을 재검토하세요.';
    } else {
      return '⚠️ 개선이 필요합니다. 전문가와 상담하세요.';
    }
  }
  ...
  analyzeHormoneEffects() {
    ...
    if (latest.estrogenLevel && latest.estrogenLevel >= 100) {
      effects.physical.push('✅ 에스트로겐 수치 적정 - 신체 여성화 진행 중');
    } else if (latest.estrogenLevel) {
      effects.physical.push('⚠️ 에스트로겐 수치 낮음 - 여성화 진행 느림');
    }
    
    if (latest.testosteroneLevel && latest.testosteroneLevel < 50) {
      effects.physical.push('✅ 테스토스테론 잘 억제됨 - 남성화 역전 중');
    } else if (latest.testosteroneLevel) {
      effects.physical.push('⚠️ 테스토스테론 억제 부족 - 항안드로겐 조정 필요');
    }
    ...
    if (latest.cupSize && latest.chest) {
      ...
      if (diff > 5) {
        effects.physical.push('✅ 가슴 발달 진행 중');
      }
    }
    ...
    if (this.measurements.length >= 2) {
      ...
      if (latest.muscleMass && previous.muscleMass && latest.muscleMass < previous.muscleMass) {
        effects.physical.push('ℹ️ 근육량 감소 - 정상적인 여성화 과정');
      }
    }
    ...
    if (latest.libido !== undefined) {
      if (latest.libido < 2) {
        effects.sexual.push('ℹ️ 성욕 감소 - 정상적인 항안드로겐 효과');
      }
    }
  ...
  _getFeminizationMessage(score) {
    if (score >= 85) {
      return '🎉 훌륭합니다! 여성화가 매우 잘 진행되고 있습니다!';
    } else if (score >= 70) {
      return '✨ 여성화가 순조롭게 진행 중입니다!';
    } else if (score >= 50) {
      return '📈 중간 정도 진행되었습니다. 계속 노력하세요!';
    } else if (score >= 30) {
      return '🌱 초기 단계입니다. 시간과 인내가 필요합니다.';
    } else {
