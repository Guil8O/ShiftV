...
export class HealthEvaluator {
  ...
  getWHREvaluation(category, mode) {
    if (mode === 'mtf') {
      switch (category) {
        case 'very_feminine':
          return this._t({ ko: '🎉 매우 여성적인 체형입니다!', en: '🎉 Very feminine body shape!', ja: '🎉 とても女性的な体型です！' });
        case 'feminine':
          return this._t({ ko: '✨ 여성적인 체형입니다.', en: '✨ Feminine body shape.', ja: '✨ 女性的な体型です。' });
        case 'neutral':
          return this._t({
      ...
      switch (category) {
        case 'very_masculine':
          return this._t({ ko: '🎉 매우 남성적인 체형입니다!', en: '🎉 Very masculine body shape!', ja: '🎉 とても男性的な体型です！' });
        case 'masculine':
          return this._t({ ko: '✨ 남성적인 체형입니다.', en: '✨ Masculine body shape.', ja: '✨ 男性的な体型です。' });
        case 'neutral':
          return this._t({
  ...
  getShoulderWaistEvaluation(category, mode) {
    if (mode === 'ftm') {
      switch (category) {
        case 'very_masculine':
          return this._t({
            ko: '🎉 매우 넓은 어깨! V자 체형 완성!',
            en: '🎉 Very broad shoulders! V-shape achieved!',
            ja: '🎉 とても広い肩幅！V字体型の完成！'
          });
        case 'masculine':
          return this._t({ ko: '✨ 남성적인 어깨입니다.', en: '✨ Masculine shoulders.', ja: '✨ 男性的な肩です。' });
        case 'neutral':
          return this._t({
            ko: '진행 중 - 어깨 운동을 늘리세요.',
      ...
      switch (category) {
        case 'very_feminine':
          return this._t({ ko: '🎉 여성적인 어깨 비율입니다!', en: '🎉 Feminine shoulder ratio!', ja: '🎉 女性的な肩の比率です！' });
        case 'feminine':
          return this._t({ ko: '✨ 좋은 비율입니다.', en: '✨ Great ratio.', ja: '✨ 良い比率です。' });
        case 'neutral':
          return this._t({
    ...
    if (measurement.bodyFatPercentage) {
      ...
      if (this.mode === 'mtf') {
        if (measurement.bodyFatPercentage < 20) {
          ...
          fatCategory = 'optimal';
          healthStatus = this._t({ ko: '✅ 이상적인 체지방률입니다!', en: '✅ Ideal body fat percentage!', ja: '✅ 理想的な体脂肪率です！' });
        ...
        if (measurement.bodyFatPercentage < 10) {
          ...
          healthStatus = this._t({ ko: '⚠️ 너무 낮습니다. 건강에 주의하세요.', en: '⚠️ Too low. Watch your health.', ja: '⚠️ 低すぎます。健康に注意してください。' });
        } else if (measurement.bodyFatPercentage < 15) {
          fatCategory = 'optimal';
          healthStatus = this._t({ ko: '✅ 이상적인 남성적 체지방률입니다!', en: '✅ Ideal male body fat percentage!', ja: '✅ 理想的な男性の体脂肪率です！' });
        ...
        if (measurement.bodyFatPercentage < 16 || measurement.bodyFatPercentage > 24) {
          ...
          healthStatus = this._t({ ko: '✅ 균형 잡힌 체지방률입니다.', en: '✅ Balanced body fat percentage.', ja: '✅ バランスの取れた体脂肪率です。' });
        }
      }
  ...
  getMuscleMassEvaluation(category, mode) {
    if (mode === 'ftm') {
      switch (category) {
        case 'very_high':
          return this._t({ ko: '💪 매우 높은 근육량!', en: '💪 Very high muscle mass!', ja: '💪 とても高い筋肉量！' });
        case 'high':
          return this._t({ ko: '✨ 좋은 근육량입니다.', en: '✨ Good muscle mass.', ja: '✨ 良い筋肉量です。' });
        case 'normal':
      ...
      switch (category) {
        ...
        case 'normal':
          return this._t({ ko: '✨ 적절한 수준입니다.', en: '✨ Appropriate level.', ja: '✨ 適切なレベルです。' });
        case 'low':
          return this._t({
            ko: '⚠️ 근육량이 너무 낮습니다. 건강을 위해 약간의 근력 운동이 필요합니다.',
            en: '⚠️ Muscle mass is too low. Some strength training is recommended for health.',
            ja: '⚠️ 筋肉量が低すぎます。健康のために軽い筋トレが推奨されます。'
          });
      }
    }
    return this._t({ ko: '보통 수준입니다.', en: 'Average level.', ja: '普通のレベルです。' });
    ...
    if (this.mode === 'mtf') {
      ...
      if (measurement.estrogenLevel) {
        ...
        if (eLevel < 100) {
          ...
          status = 'optimal';
          recommendation = this._t({ ko: '이상적인 범위입니다 ✅', en: 'Ideal range ✅', ja: '理想的な範囲です ✅' });
        } else if (eLevel < 300) {
          status = 'high';
          recommendation = this._t({
            ko: '약간 높습니다. 혈전 위험 주의',
          ...
          recommendation = this._t({
            ko: '⚠️ 위험 수준. 즉시 의사 상담',
            en: '⚠️ Risk level. Consult a doctor immediately.',
            ja: '⚠️ 危険水準。直ちに医師に相談してください。'
          });
        }
      ...
      if (measurement.testosteroneLevel) {
        ...
        if (tLevel < 20) {
          ...
          recommendation = this._t({
            ko: '⚠️ 너무 낮습니다. 피로감 주의',
            en: '⚠️ Too low. Watch for fatigue.',
            ja: '⚠️ 低すぎます。疲労に注意。'
          });
        } else if (tLevel < 50) {
          status = 'optimal';
          ...
          status = 'too_high';
          recommendation = this._t({
            ko: '⚠️ 억제 실패. 항안드로겐 증량 필요',
            en: '⚠️ Suppression failed. Increase anti-androgens may be needed.',
            ja: '⚠️ 抑制不十分。抗アンドロゲン増量が必要かもしれません。'
          });
        }
      ...
      if (measurement.estrogenLevel) {
        ...
        if (eLevel < 30) {
          status = 'optimal';
          recommendation = this._t({ ko: '이상적으로 억제되었습니다 ✅', en: 'Optimally suppressed ✅', ja: '理想的に抑制されています ✅' });
        } else if (eLevel < 50) {
          status = 'moderate';
  ...
  getScoreMessage(score, mode) {
    ...
    if (score >= 80) {
      return this._t(
        { ko: '🎉 훌륭합니다! {direction} 진행이 매우 잘 되고 있습니다!', en: '🎉 Excellent! Your {direction} is going very well!', ja: '🎉 素晴らしいです！{direction}の進行はとても順調です！' },
        { direction }
      );
    } else if (score >= 60) {
      return this._t(
        { ko: '✨ 좋습니다! {direction}가 순조롭게 진행 중입니다.', en: '✨ Great! Your {direction} is progressing smoothly.', ja: '✨ 良い感じです！{direction}は順調に進んでいます。' },
        { direction }
      );
    } else if (score >= 40) {
      return this._t({ ko: '📈 진행 중입니다. 꾸준히 노력하세요!', en: '📈 In progress. Keep it up!', ja: '📈 進行中です。継続して頑張りましょう！' });
    } else if (score >= 20) {
      return this._t({ ko: '🌱 초기 단계입니다. 시간이 필요합니다.', en: '🌱 Early stage. It takes time.', ja: '🌱 初期段階です。時間が必要です。' });
    } else {
