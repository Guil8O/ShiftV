...
export class FTMAnalyzer {
  ...
  analyzeBodyReshaping() {
    ...
    if (latest.shoulder && first.shoulder) {
      changes.shoulder = {
        ...
        evaluation: latest.shoulder > first.shoulder ? '✅ 증가 중 (좋음)' : '⚠️ 정체 또는 감소'
      };
    }
    ...
    if (latest.hips && first.hips) {
      changes.hips = {
        ...
        evaluation: latest.hips < first.hips ? '✅ 감소 중 (좋음)' : '⚠️ 증가 또는 정체'
      };
    }
    ...
    if (latest.waist && latest.hips && first.waist && first.hips) {
      ...
      changes.whr = {
        ...
        evaluation: latestWHR > firstWHR ? '✅ 남성적으로 변화 중' : '⚠️ 개선 필요'
      };
    }
    ...
    if (latest.arm && first.arm) {
      changes.arm = {
        ...
        evaluation: latest.arm > first.arm ? '✅ 증가 중 (좋음)' : '⚠️ 근력 운동 필요'
      };
    }
  ...
  analyzeHormoneEffects() {
    ...
    if (latest.testosteroneLevel && latest.testosteroneLevel >= 300) {
      effects.physical.push('✅ 테스토스테론 수치 적정 - 남성화 진행 중');
    } else if (latest.testosteroneLevel) {
      effects.physical.push('⚠️ 테스토스테론 수치 낮음 - 남성화 진행 느림');
    }
    ...
    if (this.measurements.length >= 2) {
      ...
      if (latest.muscleMass && previous.muscleMass && latest.muscleMass > previous.muscleMass) {
        effects.physical.push('✅ 근육량 증가 중 - 테스토스테론 효과');
      }
    }
    
    // 생리 중단 (예상되는 효과)
    effects.reproductive.push('ℹ️ 생리 중단 - 정상적인 테스토스테론 효과');
    
    // 잠재적 부작용
    if (latest.testosteroneLevel && latest.testosteroneLevel > 800) {
      effects.sideEffects.push('⚠️ T 수치 높음 - 여드름, 공격성 증가 주의');
    }
  ...
  _getMasculinizationMessage(score) {
    if (score >= 85) {
      return '🎉 훌륭합니다! 남성화가 매우 잘 진행되고 있습니다!';
    } else if (score >= 70) {
      return '✨ 남성화가 순조롭게 진행 중입니다!';
    } else if (score >= 50) {
