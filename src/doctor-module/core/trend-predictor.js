...
export class TrendPredictor {
  ...
  getProgressMessage(status, achieved, total) {
    switch (status) {
      case 'all_achieved':
        return `🎉 모든 목표 달성! (${achieved}/${total})`;
      case 'more_than_half':
        return `💪 절반 이상 달성! (${achieved}/${total})`;
      case 'in_progress':
        return `📈 진행 중 (${achieved}/${total} 달성)`;
      case 'just_started':
        return `🌱 시작 단계 (${achieved}/${total} 달성)`;
      case 'no_targets':
  ...
  getChangeRateMessage(evaluation, change) {
    ...
    switch (evaluation) {
      ...
      case 'good':
        return `✅ 이상적인 속도로 ${direction}하고 있습니다. (주당 ${absChange.toFixed(2)})`;
      case 'rapid':
        return `⚠️ 매우 빠르게 ${direction}하고 있습니다. (주당 ${absChange.toFixed(2)}) 건강에 주의하세요.`;
      default:
        return '';
    }
  }
